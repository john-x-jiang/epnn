...
class DomainInvariantDynamics_1(BaseModel):
    def __init__(self,
                 num_channel,
                 latent_dim,
                 obs_dim,
    ...
    def forward(self, x, heart_name, label=None, D=None, D_label=None):
        ...
        z_s = self.signal_encoder(x, heart_name, y)
        mu_c, logvar_c = self.get_latent_domain(z_s, heart_name)
        z_c = self.reparameterization(mu_c, logvar_c)

        z_0 = self.get_latent_initial(y, heart_name)

        z = self.time_modeling(z_s, z_0, z_c)
        x = self.decoder(z, heart_name)
...
class DetDomainInvariantDynamics(BaseModel):
    def __init__(self,
                 num_channel,
                 latent_dim,
                 obs_dim,
